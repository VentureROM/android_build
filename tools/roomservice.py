...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from VentureROM Github (http://github.com/VentureROM)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:VentureROM+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
