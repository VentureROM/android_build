...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${VENTURE_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the VentureROM github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var VENTURE_VERSION)
        ZIPFILE=venture-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.venture.device | grep -q "$VENTURE_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $VENTURE_BUILD, run away!"
    fi
}
...
function cmremote()
{
    ...
    CMUSER=$(git config --get review.review.venture/ogenmod.org.username)
    if [ -z "$CMUSER" ]
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.venture.device | grep -q "$VENTURE_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $VENTURE_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/venture/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
